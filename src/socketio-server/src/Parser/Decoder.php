...
class Decoder
{
    ...
    public function decode(string $payload): Packet
    {
        ...
        if ($payload[$currentIndex + 1] === '/') {
            ...
            $nspStart = strpos($payload, '/');
            $nspEnd = strpos($payload, ',');
            $queryStart = strpos($payload, '?');

            if ($nspEnd === false) {
                $nsp = substr($payload, $nspStart);
                goto _out;
            }
            ...
            if ($queryStart !== false) {
                $queryStr = substr($payload, $queryStart + 1, $nspEnd - $queryStart - 1);
     ...
     */
    public function backupDecode(string $payload): Packet
    {
        if (! $payload) {
