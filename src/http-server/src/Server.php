...
class Server implements OnRequestInterface, MiddlewareInitializerInterface
{
    ...
    public function onRequest($request, $response): void
    {
        try {
            ...
            $this->option?->isEnableRequestLifecycle() && $this->event?->dispatch(new RequestReceived(request: $psr7Request, response: $psr7Response, server: $this->serverName));
            ...
            if (isset($psr7Request) && $this->option?->isEnableRequestLifecycle()) {
                defer(fn () => $this->event?->dispatch(new RequestTerminated(request: $psr7Request, response: $psr7Response ?? null, exception: $throwable ?? null, server: $this->serverName)));

                $this->event?->dispatch(new RequestHandled(request: $psr7Request, response: $psr7Response ?? null, exception: $throwable ?? null, server: $this->serverName));
            }
