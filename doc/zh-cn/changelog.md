...
- [#1734](https://github.com/hyperf/hyperf/pull/1734) 修复模型多态查询，关联关系为空时，也会查询SQL的BUG。
- [#1739](https://github.com/hyperf/hyperf/pull/1739) 修复 OSS HOOk 位运算错误，导致 resource 判断不准确的BUG。
- [#1743](https://github.com/hyperf/hyperf/pull/1743) 修复 `grafana.json` 中，`refId` 字段设置有误的BUG。
- [#1748](https://github.com/hyperf/hyperf/pull/1748) 修复 `AMQP` 组件，使用其他连接池时，对应的 `concurrent.limit` 配置不生效的BUG。
- [#1750](https://github.com/hyperf/hyperf/pull/1750) 修复连接池组件，连接关闭失败，会导致计数有误的BUG。
- [#1754](https://github.com/hyperf/hyperf/pull/1754) 修复 BASE 服务，启动提示不准确的BUG。
- [#1764](https://github.com/hyperf/hyperf/pull/1764) 修复时间值为 null 时，datatime 验证器执行失败的BUG。
- [#1769](https://github.com/hyperf/hyperf/pull/1769) 修复 `socketio-server` 中，客户端初始化断开连接操作的提示错误。
