# DAG

`hyperf/dag` é uma biblioteca leve de orquestração de tarefas em grafo acíclico dirigido (**D**irected **A**cyclic **G**raph).

## Cenários

Suponha que temos uma série de tarefas a executar.

- Se houver dependência entre elas, podem ser executadas sequencialmente.
- Se não dependerem umas das outras, podemos optar por executá-las concorrentemente para acelerar a execução.
- Também há um estado intermediário entre os dois: algumas tarefas têm dependências, enquanto outras podem ser executadas concorrentemente.

Podemos resolver esse terceiro cenário complexo abstraindo-o em um `DAG`.

## Instalação

```bash
composer require hyperf/dag
```

## Exemplo

[![](https://mermaid.ink/img/eyJjb2RlIjoic3RhdGVEaWFncmFtLXYyXG4gICAgWypdIC0tPiBBXG4gICAgQSAtLT4gQlxuICAgIEEgLS0-IENcbiAgICBBIC0tPiBEXG4gICAgRCAtLT4gR1xuICAgIEMgLS0-IEdcbiAgICBDIC0tPiBGXG4gICAgQiAtLT4gRlxuICAgIEIgLS0-IEVcbiAgICBCIC0tPiBIXG4gICAgSCAtLT4gSVxuICAgIEUgLS0-IElcbiAgICBGIC0tPiBJXG4gICAgRyAtLT4gSVxuICAgIEkgLS0-)](https://mermaid-js.github.io/mermaid-live-editor/#/edit/eyJjb2RlIjoic3RhdGVEaWFncmFtLXYyXG4gICAgWypdIC0tPiBBXG4gICAgQSAtLT4gQlxuICAgIEEgLS0-IENcbiAgICBBIC0tPiBEXG4gICAgRCAtLT4gR1xuICAgIEMgLS0-IEdcbiAgICBDIC0tPiBGXG4gICAgQiAtLT4gRlxuICAgIEIgLS0-IEVcbiAgICBCIC0tPiBIXG4gICAgSCAtLT4gSVxuICAgIEUgLS0-IElcbiAgICBGIC0tPiBJXG4gICAgRyAtLT4gSVxuICAgIEkgLS0-)

Suponha que temos uma série de tarefas, cuja topologia é mostrada acima, os vértices representam tarefas e as arestas representam dependências. (B, C, D só podem ser concluídas depois que A for concluída, e H, E, F... só podem ser concluídas depois que B for concluída.)

Com `hyperf/dag`, o `DAG` pode ser construído e executado da seguinte forma.

```php
<?php
$dag = new \Hyperf\Dag\Dag();
$a = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "A\n";});
$b = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "B\n";});
$c = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "C\n";});
$d = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "D\n";});
$e = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "E\n";});
$f = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "F\n";});
$g = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "G\n";});
$h = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "H\n";});
$i = \Hyperf\Dag\Vertex::make(function() {sleep(1); echo "I\n";});
$dag->addVertex($a)
    ->addVertex($b)
    ->addVertex($c)
    ->addVertex($d)
    ->addVertex($e)
    ->addVertex($f)
    ->addVertex($g)
    ->addVertex($h)
    ->addVertex($i)
    ->addEdge($a, $b)
    ->addEdge($a, $c)
    ->addEdge($a, $d)
    ->addEdge($b, $h)
    ->addEdge($b, $e)
    ->addEdge($b, $f)
    ->addEdge($c, $f)
    ->addEdge($c, $g)
    ->addEdge($d, $g)
    ->addEdge($h, $i)
    ->addEdge($e, $i)
    ->addEdge($f, $i)
    ->addEdge($g, $i);
    
// need to be executed in a coroutine environment
$dag->run(); 

```

Saída:

```php
// 1s afterwards
A
// 2s afterwards
D
C
B
// 3s afterwards
G
F
E
H
// 4s afterwards
I
```

> O DAG agenda as tarefas com base na execução mais precoce possível. Tente ajustar o tempo no ponto B para 2 segundos, e você verá que B e G são concluídas juntas.

## Acessar os resultados da etapa anterior

Cada tarefa pode receber um parâmetro em array contendo os resultados de todas as dependências anteriores. Depois que o `DAG` for executado, ele também retornará um array da mesma estrutura, incluindo o resultado de execução de cada etapa.

```php
<?php
$dag = new \Hyperf\Dag\Dag();
$a = \Hyperf\Dag\Vertex::make(function() {return 1;});
$b = \Hyperf\Dag\Vertex::make(function($results) use ($a) {
    return $results[$a->key] + 1;
});
$results = $dag->addVertex($a)->addVertex($b)->addEdge($a, $b)->run();
assert($results[$a->key] === 1);
assert($results[$b->key] === 2);
```

## Definir uma tarefa

Na documentação acima, usamos uma closure para definir uma tarefa. O formato é o seguinte.

```php
// The second parameter of Vertex::make is an optional parameter, which is the key of vertex, that is, the key value of the result array.
\Hyperf\Dag\Vertex::make(function() { return 'hello'; }, "greeting");
```

Além de usar funções closure para definir tarefas, você também pode usar uma classe que implementa a interface `\Hyperf\Dag\Runner` e convertê-la em um vertex através de `Vertex::of`.

```php
class MyJob implements \Hyperf\Dag\Runner {
    public function run($results = []) {
        return 'hello';
    }
}

\Hyperf\Dag\Vertex::of(new MyJob(), "greeting");
```

O próprio `\Hyperf\Dag\Dag` também implementa a interface `\Hyperf\Dag\Runner`, então ele pode ser aninhado.

```php
<?php
// namespace omitted
$a = Vertex::make(function () { return 1;});
$b = Vertex::make(function () { return 2;});
$c = Vertex::make(function () { return 3;});

$nestedDag = new Dag();
$nestedDag->addVertex($a)->addVertex($b)->addEdge($a, $b);
$d = Vertex::of($nestedDag);

$superDag = new Dag();
$superDag->addVertex($c)->addVertex($d)->addEdge($c, $d);
$superDag->run();
```

## Controlar o número de concorrência
A classe `\Hyperf\Dag\Dag` fornece o método `setConcurrency(int n)` para controlar o número máximo de concorrência. O padrão é 10.
