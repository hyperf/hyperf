...
`hyperf/nano` é um pacote que reduz o Hyperf a um único arquivo.
...
* Zero config.
* Estilo closure.
* Suporta todas as funcionalidades do Hyperf, exceto annotations.
...
> Além de closure, todos os métodos $app->addXXX() também aceitam nome de classe como argumento. Você pode passar qualquer classe correspondente do hyperf.
...
Para executar esse command, execute
```bash
php index.php echo
```
