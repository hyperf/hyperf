...
O Hyperf fornece um encapsulamento do WebSocket Client. O WebSocket Server pode ser acessado através do componente [hyperf/websocket-client](https://github.com/hyperf/websocket-client).
...
Por padrão, o objeto `Client` criado será encerrado por `defer`. Se isso não for desejado, o auto-close pode ser desabilitado configurando `$autoClose` como `false` ao instanciar o `Client`.
