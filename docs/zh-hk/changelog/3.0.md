...
- [#6297](https://github.com/hyperf/hyperf/pull/6297) 新增接口 `Hyperf\Crontab\Mutex\ServerNodeInterface` 用於使用 `Crontab` 的 `onOneServer` 功能時，自定義唯一鍵。
- [#6300](https://github.com/hyperf/hyperf/pull/6300) 新增配置 `rate_limit.storage.options.pool` 用於使用限流器時，設置 `Redis` 連接池。
- [#6306](https://github.com/hyperf/hyperf/pull/6306) 為 `RPC` 服務增加請求級別生命週期。

## 修復

- [#6298](https://github.com/hyperf/hyperf/pull/6298) 修復 `Hyperf\Framework\Logger\StdoutLogger` 中的 `context` 參數不支持複雜對象的問題。
- [#6303](https://github.com/hyperf/hyperf/pull/6303) 修復 `PDO` 鏈接在超時時，不會被重試的問題。

## 優化

- [#6302](https://github.com/hyperf/hyperf/pull/6302) 使用 `command` 模式調用定時任務時，設置 `setCatchExceptions` 為 false。
- [#6315](https://github.com/hyperf/hyperf/pull/6315) 優化鏈路追蹤組件從上下文讀取 `Tracer` 時，不會多一次實例化的問題。
...
- [#6273](https://github.com/hyperf/hyperf/pull/6273) 為 `Hyperf\HttpServer\Router\Dispatched` 增加 `serverName` 參數。
...
- [#6236](https://github.com/hyperf/hyperf/pull/6236) 為 `GenerateModelIDEVisitor` 增加 `unionType` 支持。
- [#6246](https://github.com/hyperf/hyperf/pull/6246) [#6252](https://github.com/hyperf/hyperf/pull/6252) 增加 `Crontab` 生命週期事件。
- [#6249](https://github.com/hyperf/hyperf/pull/6249) 為 `Crontab` 增加 `closure` 執行類型。

## 優化

- [#6239](https://github.com/hyperf/hyperf/pull/6239) 優化 `AMQP` 代碼，替換一些即將廢棄的方法，並增加 `BeforeConsume` 事件。
- [#6250](https://github.com/hyperf/hyperf/pull/6250) 優化 `generateModelIdeVisitor` 註釋的生成。
- [#6253](https://github.com/hyperf/hyperf/pull/6253) 當收集類反射數據失敗時，輸出錯誤信息到終端。
- [#6256](https://github.com/hyperf/hyperf/pull/6256) 優化 `hyperf/translation` 部分代碼，為成員變量增加類型限制。

# v3.0.41 - 2023-10-27

## 新增

- [#6224](https://github.com/hyperf/hyperf/pull/6224) 為 `Response::xml()` 等方法，增加 `charset` 參數。
- [#6232](https://github.com/hyperf/hyperf/pull/6232) 為 `Hyperf\HttpServer\Router\Dispatched` 增加 `isNotFound` 方法。
- [#6233](https://github.com/hyperf/hyperf/pull/6233) 為 `hyperf/command` 組件的 `AfterExecute` 事件，增加 `exception` 入參。

## 優化

- [#6226](https://github.com/hyperf/hyperf/pull/6226) 當 `grpc` 連接關閉時，不再發送數據。
...
- [#6209](https://github.com/hyperf/hyperf/pull/6209) 為 `Redis` [哨兵模式](https://github.com/phpredis/phpredis/blob/develop/sentinel.md#examples-for-version-60-or-later) 增加 `v6` 版本的支持。
