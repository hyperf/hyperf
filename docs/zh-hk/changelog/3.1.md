...
- [#7764](https://github.com/hyperf/hyperf/pull/7764) 修復 `Hyperf\Coordinator\Timer::tick()` 因為使用了 `Hyperf\Coroutine\wait()` 導致 10s 超時的問題。
- [#7791](https://github.com/hyperf/hyperf/pull/7791) 修復 `Hyperf\Validation\Concerns\ValidatesAttributes::validateMultipleOf()` 不支持最新 `brick/math` 的問題。

## 優化

- [#7791](https://github.com/hyperf/hyperf/pull/7791) 優化 `Hyperf\Tracer\Adapter\Reporter\HttpClientFactory` 的 `Content-Type` 數據類型為 `string`。

# v3.1.70 - 2026-06-15

## 優化

- [#7761](https://github.com/hyperf/hyperf/pull/7761) 優化方法 `Hyperf\Coordinator\Timer::tick()` 的執行邏輯運行於 `Hyperf\Coroutine\wait()` 方法中，以確保運行在獨立的協程環境裏。
- [#7761](https://github.com/hyperf/hyperf/pull/7761) 優化方法 `wait()` 優先檢查 `Hyperf\Coroutine\Waiter` 是否存在容器中。

# v3.1.69 - 2026-05-09

## 修復

- [#7731](https://github.com/hyperf/hyperf/pull/7731) 修復了 Hyperf\HttpServer\Response 的 Macroable 無法工作的 bug。
- [#7734](https://github.com/hyperf/hyperf/pull/7734) 修復了在長生命週期環境下使用 Redis::pipeline() 時內存泄漏的 bug。
- [#7736](https://github.com/hyperf/hyperf/pull/7736) 修復了 RedisDriver 中 keys 方法在 sMembers 返回非數組值時無法返回數組的 bug。
- [#7737](https://github.com/hyperf/hyperf/pull/7737) 修復了 PostgresProcessor 在 columns 為 null 時無法處理索引和外鍵的 bug。
- [#7745](https://github.com/hyperf/hyperf/pull/7745) 修復了使用 Swoole 6.2.0 時 getUri()->getHost() 返回服務器 IP 而非域名的 bug，原因是 getUriFromGlobals() 中 server_addr 的優先級高於 header['host']。

# v3.1.68 - 2026-04-21

## 修復

- [#7738](https://github.com/hyperf/hyperf/pull/7738) 修復 `Hyperf\Database\Concerns\BuildsQueries::getOriginalColumnNameForCursorPagination` 無法正常使用的BUG。
- [#7739](https://github.com/hyperf/hyperf/pull/7739) 修復使用 `nacos` 時，因為服務不存在，導致無法啓動的問題。
- [#7740](https://github.com/hyperf/hyperf/pull/7740) 修復 `Aws\Handler\GuzzleV6\GuzzleHandler` 找不到的問題。

## 優化

- [#7741](https://github.com/hyperf/hyperf/pull/7741) 優化方法 `Hyperf\Amqp\Message\Message::getTypeString()` 避免成員變量與 `getter` 衝突的問題。統一使用 `getter` 讀取。

# v3.1.67 - 2026-02-24

## 新增

- [#7459](https://github.com/hyperf/hyperf/pull/7459) 使用驗證器規則 `exists` 和 `unique` 時，支持使用形如 `App\\Model\\User` 的模型寫法。
- [#7555](https://github.com/hyperf/hyperf/pull/7555) 新增 `Hyperf\Crontab\Mutex\RedisServerMutexByNodeName`。
- [#7607](https://github.com/hyperf/hyperf/pull/7607) 為 `hyperf/database` 增加新的模型自定義類型轉換器 `AsCollection`。
- [#7648](https://github.com/hyperf/hyperf/pull/7648) 為 `nacos` 支持阿里雲 `mse` 簽名。

## 優化

- [#7676](https://github.com/hyperf/hyperf/pull/7676) 優化 `hyperf/tracer` 組件的客户端標籤寫法。

## 修復

- [#7703](https://github.com/hyperf/hyperf/pull/7703) 修復 `hyperf/cache` 內存適配器不支持默認 `ttl` 的問題。
- [#7705](https://github.com/hyperf/hyperf/pull/7705) 修復 `Hyperf\Database\Query\Builder::eachById` 無法正常使用的問題。
- [#7712](https://github.com/hyperf/hyperf/pull/7712) 修復 `classes` 等掃描緩存解析失敗導致報錯的問題。

# v3.1.66 - 2026-01-25

## 新增

- [#7667](https://github.com/hyperf/hyperf/pull/7667) [#7672](https://github.com/hyperf/hyperf/pull/7672) 為 `hyperf/grpc-client` 新增參數 `client_count` 用來支持創建多個客户端。
- [#7668](https://github.com/hyperf/hyperf/pull/7668) 為 `Hyperf\Guzzle\PoolHandler` 新增參數 `$isCookiePersistent` 用來支持是否持久化 `Cookies`。
- [#7670](https://github.com/hyperf/hyperf/pull/7670) 為文件生成命令增加 `--path` 選項，用來指定生成位置。
- [#7684](https://github.com/hyperf/hyperf/pull/7684) 新增方法 `Hyperf\Database\Schema\Builder::withoutForeignKeyConstraints()`。
- [#7693](https://github.com/hyperf/hyperf/pull/7693) 為 `ForeignKeyDefinition` 新增方法 `cascadeOnDelete()`, `restrictOnDelete()`, `noActionOnDelete()`, `restrictOnUpdate()`, `nullOnUpdate()`, `noActionOnUpdate()`。
- [#7698](https://github.com/hyperf/hyperf/pull/7698) 新增方法 `Pool::flushAll()` 和 `PoolFactory::flushAll()`。
- [#7699](https://github.com/hyperf/hyperf/pull/7699) 為 `QueryBuilder` 新增方法 `whereJsonContainsKey()`, `orWhereJsonContainsKey()`, `whereJsonDoesntContainKey()`, `orWhereJsonDoesntContainKey()`。

## 優化

- [#7675](https://github.com/hyperf/hyperf/pull/7675) 為 `metric` 組件刪除多餘的定時器清理代碼。
- [#7687](https://github.com/hyperf/hyperf/pull/7687) 為 `JSON-RPC` 組件增加默認捕獲 `ValidationException` 的邏輯。

## 修復

- [#7669](https://github.com/hyperf/hyperf/pull/7669) 修復 `PostgreSQL` 組件中 `boolean` 格式問題。
- [#7682](https://github.com/hyperf/hyperf/pull/7682) 修復 `Blueprint::foreign()` 返回值註釋錯誤。

# v3.1.65 - 2025-12-04

## 新增

- [#7585](https://github.com/hyperf/hyperf/pull/7585) 新增事件 `Hyperf\Kafka\Event\FailToAck` 和 `Hyperf\Kafka\Event\FailToRequeue`。
- [#7632](https://github.com/hyperf/hyperf/pull/7632) 增加 `thecodingmachine/graphqlite:^8.1` 的支持。
- [#7644](https://github.com/hyperf/hyperf/pull/7644) 新增 `Str::uuidv7()`。

## 優化

- [#7649](https://github.com/hyperf/hyperf/pull/7649) 為 `Hyperf\Metric\Listener\OnBeforeHandle` 移除 `AfterExecute` 的事件監聽。

# v3.1.64 - 2025-11-12

## 新增

- [#7600](https://github.com/hyperf/hyperf/pull/7600) 新增方法 `Hyperf\Collection\Arr::sole()`。
- [#7601](https://github.com/hyperf/hyperf/pull/7601) 增加異步隊列消息執行結果枚舉 `Hyperf\AsyncQueue\Result`。

## 優化

- [#7604](https://github.com/hyperf/hyperf/pull/7604) 優化異步隊列 `AsyncQueueMessage` 註解支持定義在 `protected` 和 `private` 方法上。
- [#7611](https://github.com/hyperf/hyperf/pull/7611) 為 `Console::command()` 增加註釋 `@param-closure-this`，用來獲得更好的 `ide` 代碼提示。
- [#7615](https://github.com/hyperf/hyperf/pull/7615) 把異步隊列的消費者進程中的 `$queue` 參數重命名為 `$pool`。

## 修復

- [#7596](https://github.com/hyperf/hyperf/pull/7596) 修復使用驗證器時，使用了不存在的場景值，導致報錯的問題。

# v3.1.63 - 2025-10-30

## 修復

- [#7536](https://github.com/hyperf/hyperf/pull/7536) 修復 `Hyperf\Validation\ValidationRuleParser::parseParameters()` 不支持 `PHP 8.4` 的問題。
- [#7543](https://github.com/hyperf/hyperf/pull/7543) 修復 `nacos` 組件代碼沒有嚴格符合 `psr4` 標準的問題。
- [#7554](https://github.com/hyperf/hyperf/pull/7554) 修復參數 `Redis` 集羣 `context` 參數在沒有設置密碼時，不生效的問題。
- [#7561](https://github.com/hyperf/hyperf/pull/7561) 重置 `Nacos\Application` 的構造函數參數名。

## 新增

- [#7553](https://github.com/hyperf/hyperf/pull/7553) 對 `hyperf/testing` 組件增加 `PATCH` 方法的支持。

## 優化

- [#7511](https://github.com/hyperf/hyperf/pull/7511) 優化 `hyperf/database` 的註釋。
- [#7537](https://github.com/hyperf/hyperf/pull/7537) 對 `swagger` 組件，使用 `priorityMiddleware` 替代 `middleware`。

# v3.1.62 - 2025-10-11

## 新增

- [#7469](https://github.com/hyperf/hyperf/pull/7469) 使用參數驗證器時，支持通過場景值覆寫參數驗證規則。
- [#7531](https://github.com/hyperf/hyperf/pull/7531) 為腳本 `migrate:rollback` 新增參數 `batch`。
- [#7545](https://github.com/hyperf/hyperf/pull/7545) 運行腳本最後自動執行 `CoordinatorManager::until(Constants::WORKER_EXIT)->resume();`。

## 修復

- [#7547](https://github.com/hyperf/hyperf/pull/7547) 修復 `kafka` 消費者配置成員變量 `$pool` 時，無法正常使用的問題。

# v3.1.61 - 2025-09-04

## 新增

- [#7467](https://github.com/hyperf/hyperf/pull/7467) 為 Nacos SDK 支持 v2 和 v3 版本。
- [#7476](https://github.com/hyperf/hyperf/pull/7476) 為 `Hyperf\Support\Fluent` 新增 trait `InteractsWithData` 支持。
...
- [#7483](https://github.com/hyperf/hyperf/pull/7483) 新增 `Hyperf\Database\Model\Relations\BelongsTo::dissociate()` 的別名 `disassociate()`。
...
- [#7487](https://github.com/hyperf/hyperf/pull/7487) 新增模型對 `ArrayObject` 類型的轉換器 `Hyperf\Database\Model\Casts\AsArrayObject`。
- [#7492](https://github.com/hyperf/hyperf/pull/7492) 為 `Hyperf\Pipeline\Pipeline` 新增 trait `Macroable` 的支持。
...
- [#7501](https://github.com/hyperf/hyperf/pull/7501) 為 `Hyperf\Macroable\Macroable::macro()` 增加引用驗證，避免使用引用導致代碼執行結果不達預期的情況。

# v3.1.60 - 2025-08-02

## 修復

- [#7447](https://github.com/hyperf/hyperf/pull/7447) 修復 `migrate:fresh` 無法刪除默認表的問題。
- [#7449](https://github.com/hyperf/hyperf/pull/7449) 修復方法 `Hyperf\Database\Migrations\Migrator::reset()` 不支持傳入 `string` 格式參數的問題。
...
- [#7421](https://github.com/hyperf/hyperf/pull/7421) 修復當設置 `Cookies` 中某個值為 "" 時，無法正確刪除此 `Cookie` 的問題。
- [#7424](https://github.com/hyperf/hyperf/pull/7424) 修復使用 `watcher` 組件時，`php` 二進制文件路徑帶有空格會導致報錯的問題。
- [#7427](https://github.com/hyperf/hyperf/pull/7427) 修復驗證器中 `exclude` 規則不可用的問題。

## 優化

- [#7394](https://github.com/hyperf/hyperf/pull/7394) 當使用 `Redis::transaction($callback)` 和 `Redis::transaction($callback)` 後，可以立即釋放連接到連接池中。

# v3.1.57 - 2025-06-23

## 修復

- [#7402](https://github.com/hyperf/hyperf/pull/7402) 修復使用 `pgsql-swoole` 數據庫時，參數 `connect_timeout` 無法正常工作的問題。
- [#7414](https://github.com/hyperf/hyperf/pull/7414) 修復 `sqlite` 數據庫遷移無法正常使用的問題。
- [#7420](https://github.com/hyperf/hyperf/pull/7420) 修復使用模型緩存時，`luasha` 會在 `Redis` 重啓時發生故障的問題。

## 優化

- [#7411](https://github.com/hyperf/hyperf/pull/7411) 移除 `Hyperf\Utils` 組件。

# v3.1.56 - 2025-06-05

## 修復

- [#7400](https://github.com/hyperf/hyperf/pull/7400) 使用 `pgsql` 時，修復 `gen:model` 腳本無法正常使用的問題。
- [#7410](https://github.com/hyperf/hyperf/pull/7410) 修復使用緩存模塊時，內存適配器不支持 `setMultiple` 方法的問題。

## 新增

- [#7392](https://github.com/hyperf/hyperf/pull/7392) 為 `AsCommand` 和 `ClosureCommand` 增加 `coroutine` 選項。
- [#7404](https://github.com/hyperf/hyperf/pull/7404) 為 `CacheAHead` 註解增加 `runAsync` 參數，用來異步初始化緩存操作，但會導致返回值為 `Null`。

# v3.1.55 - 2025-05-15

## Added

- [#7365](https://github.com/hyperf/hyperf/pull/7365) 為 `logger` 配置文件，增加 `callable` 方式支持。
...
- [#7363](https://github.com/hyperf/hyperf/pull/7363) 修復 `Hyperf\HttpServer\Router\RouteCollector` 中代碼註釋，解決 `IDE` 錯誤誤報。
- [#7379](https://github.com/hyperf/hyperf/pull/7379) 修復使用 `PgSQL` 的 `Migrator` 功能時，`PDO` 適配器無法正常使用的問題。

# v3.1.54 - 2025-04-27

## 新增

- [#7316](https://github.com/hyperf/hyperf/pull/7316) 為 `hyperf/resource` 組件增加支持重寫 `paginationInformation` 方法的接口。
...
- [#7334](https://github.com/hyperf/hyperf/pull/7334) 修復 `kafka` 消息生產者，在鏈接斷開後無法重連的問題。

# v3.1.53 - 2025-04-03

## 修復

- [#7309](https://github.com/hyperf/hyperf/pull/7309) 修復 `Hyperf\Testing\Client` 在使用非 `POST` 請求時，不支持 `Json` 結構的問題。
- [#7318](https://github.com/hyperf/hyperf/pull/7318) 修復 `WebSocket` 客户端不支持接收 `CloseFrame` 的問題。
- [#7329](https://github.com/hyperf/hyperf/pull/7329) 修復創建模型時，`$connection` 類型報錯的問題。
...
- [#7314](https://github.com/hyperf/hyperf/pull/7314) 增加 `hyperf/db` 組件中 `pgsql` 引擎的 `schema` 配置。
- [#7325](https://github.com/hyperf/hyperf/pull/7325) 為 `Hyperf\Di\ReflectionType` 增加註解元數據。
...
- [#7200](https://github.com/hyperf/hyperf/pull/7200) 新增方法 `Hyperf\Database\Connection::scalar()` 用於讀取滿足查詢條件的第一行數據的第一個字段值。
...
- [#7291](https://github.com/hyperf/hyperf/pull/7291) 當使用 `Model::createOrFirst` 讀到 `null` 值時，拋出異常。
...
- [#7273](https://github.com/hyperf/hyperf/pull/7273) 使用命令創建文件後，支持使用 `cursor` 編輯器自動打開對應文件。

## 優化

- [#7258](https://github.com/hyperf/hyperf/pull/7258) 優化讀取 `composer.lock` 的代碼實現。
...
- [#6556](https://github.com/hyperf/hyperf/pull/6556) 在數據庫組件時，允許為模型參數複製表達式。**_但這不是一個標準用法，儘量不要使用_**
