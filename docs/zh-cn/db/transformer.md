...
你可以使用 `gen:transformer` Artisan 命令来生成一个资源类。默认情况下生成的资源都会被放置在应用程序的 `app/Transformer` 文件夹下。资源继承自 `Hyperf\Transformer\Json\JsonResource` 类：
...
你需要在生成资源时添加 `--collection` 标志以生成一个资源集合。或者，你也可以直接在资源的名称中包含 `Collection` 表示应该生成一个资源集合。资源集合继承自 `Hyperf\Transformer\Json\ResourceCollection` 类：
...
需要安装 `hyperf/transformer-grpc` 扩展

```bash
php bin/hyperf.php gen:transformer HiReplyResource --grpc
```
...
use App\Model\User;
use App\Transformer\HiReplyResource;

class IndexController extends AbstractController
{
    public function index()
    {
        return (new HiReplyResource(User::find(1)))->toMessage();
    }
}
