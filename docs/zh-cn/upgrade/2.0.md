...
- `config/autoload/aspects.php` 文件需要存在(空文件也行), 否则会报错。【疑似BUG】
- 自定义 `process` 需要修改 `public function isEnable($server): bool` 方法，以适配新的接口。
- 自定义注解，如果使用了自定义的收集器 `Collector`，则需要将对应的 `Collector` 配置到 `annotations.scan.collectors` 中，因为开发模式下，会根据文件时间戳判断文件是否修改，然后重新收集注解。
所以，如果没有配置 `collectors`，就会导致注解只有在首次启动 `server` 时可以生效。
