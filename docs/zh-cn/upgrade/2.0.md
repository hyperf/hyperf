...
- `config/autoload/aspects.php` 文件需要存在(空文件也行), 否则会报错
- 如果使用不同环境的 `.env` 文件, Dockerfile 中执行 `php bin/hyperf.php` 缓存前, 需要执行 `cp .env.${APP_ENV}`, 否则可能因为 `.env` 中配置连接无法连接上报错
- 自定义 process 需要修改 `public function isEnable($server): bool` 方法, 兼容 2.0 版本的 api
- 自定义注解，如果使用了自定义的收集器 `Collector`，则需要将对应的 `Collector` 配置到 `annotations.scan.collectors` 中，因为开发模式下，会根据文件时间戳判断文件是否修改，然后重新收集注解。
所以，如果没有配置 collectors，就会导致只有在首次启动server时，对应自定义注解生效，而二次启动时，不会再扫描对应注解。
```php
<?php
