...
| 配置                          | 类型       | 默认值                        | 备注                                                                                                                 |
| ----------------------------- | ---------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| connect_timeout               | int｜float | 10                            | 连接超时时间（单位：秒，支持小数）制                                                                  |
| send_timeout                  | int｜float | 10                            | 发送超时时间（单位：秒，支持小数）制                                                                  |
| recv_timeout                  | int｜float | 10                            | 接收超时时间（单位：秒，支持小数）制                                                                  |
...
| interval                      | int｜float | 10                            | 未获取消息到消息时，延迟多少秒再次尝试，默认为 0 则不延迟（单位：秒，支持小数）                                      |
