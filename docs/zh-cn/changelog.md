...
- [#2331](https://github.com/hyperf/hyperf/pull/2331) Nacos组件增加授权接口。
- [#2331](https://github.com/hyperf/hyperf/pull/2331) Nacos组件增加 `nacos.enable` 配置，用于控制是否启用 `Nacos` 服务。
- [#2331](https://github.com/hyperf/hyperf/pull/2331) Nacos组件增加配置合并类型，默认使用全量覆盖。
- [#2384](https://github.com/hyperf/hyperf/pull/2384) 新增助手函数 `optional()`。

## 修改

- [#2331](https://github.com/hyperf/hyperf/pull/2331) 修复 Nacos 组件，服务或配置不存在时，会抛出异常的BUG。
- [#2356](https://github.com/hyperf/hyperf/pull/2356) [#2368](https://github.com/hyperf/hyperf/pull/2368) 修复 `pid_file` 被用户修改后，命令行 `server:start` 启动失败的BUG。
- [#2358](https://github.com/hyperf/hyperf/pull/2358) 修复验证器规则 `digits` 不支持 `int` 类型的BUG。

## 优化

- [#2359](https://github.com/hyperf/hyperf/pull/2359) 优化自定义进程，在协程风格服务下，可以更加友好的停止。
- [#2363](https://github.com/hyperf/hyperf/pull/2363) 优化 `DI` 组件，使其不需要依赖 `Config` 组件。
- [#2373](https://github.com/hyperf/hyperf/pull/2373) 优化验证器的异常捕获器，使其返回 `Response` 时，自动添加 `content-type 头。
