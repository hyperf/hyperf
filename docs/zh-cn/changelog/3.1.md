...
- [#7764](https://github.com/hyperf/hyperf/pull/7764) 修复 `Hyperf\Coordinator\Timer::tick()` 因为使用了 `Hyperf\Coroutine\wait()` 导致 10s 超时的问题。
- [#7791](https://github.com/hyperf/hyperf/pull/7791) 修复 `Hyperf\Validation\Concerns\ValidatesAttributes::validateMultipleOf()` 不支持最新 `brick/math` 的问题。

## 优化

- [#7791](https://github.com/hyperf/hyperf/pull/7791) 优化 `Hyperf\Tracer\Adapter\Reporter\HttpClientFactory` 的 `Content-Type` 数据类型为 `string`。
...
- [#7731](https://github.com/hyperf/hyperf/pull/7731) 修复了 Hyperf\HttpServer\Response 的 Macroable 无法工作的 bug。
- [#7734](https://github.com/hyperf/hyperf/pull/7734) 修复了在长生命周期环境下使用 Redis::pipeline() 时内存泄漏的 bug。
- [#7736](https://github.com/hyperf/hyperf/pull/7736) 修复了 RedisDriver 中 keys 方法在 sMembers 返回非数组值时无法返回数组的 bug。
- [#7737](https://github.com/hyperf/hyperf/pull/7737) 修复了 PostgresProcessor 在 columns 为 null 时无法处理索引和外键的 bug。
- [#7745](https://github.com/hyperf/hyperf/pull/7745) 修复了使用 Swoole 6.2.0 时 getUri()->getHost() 返回服务器 IP 而非域名的 bug，原因是 getUriFromGlobals() 中 server_addr 的优先级高于 header['host']。

# v3.1.68 - 2026-04-21

## 修复

- [#7738](https://github.com/hyperf/hyperf/pull/7738) 修复 `Hyperf\Database\Concerns\BuildsQueries::getOriginalColumnNameForCursorPagination` 无法正常使用的BUG。
- [#7739](https://github.com/hyperf/hyperf/pull/7739) 修复使用 `nacos` 时，因为服务不存在，导致无法启动的问题。
- [#7740](https://github.com/hyperf/hyperf/pull/7740) 修复 `Aws\Handler\GuzzleV6\GuzzleHandler` 找不到的问题。

## 优化

- [#7741](https://github.com/hyperf/hyperf/pull/7741) 优化方法 `Hyperf\Amqp\Message\Message::getTypeString()` 避免成员变量与 `getter` 冲突的问题。统一使用 `getter` 读取。

# v3.1.67 - 2026-02-24

## 新增

- [#7459](https://github.com/hyperf/hyperf/pull/7459) 使用验证器规则 `exists` 和 `unique` 时，支持使用形如 `App\\Model\\User` 的模型写法。
- [#7555](https://github.com/hyperf/hyperf/pull/7555) 新增 `Hyperf\Crontab\Mutex\RedisServerMutexByNodeName`。
- [#7607](https://github.com/hyperf/hyperf/pull/7607) 为 `hyperf/database` 增加新的模型自定义类型转换器 `AsCollection`。
- [#7648](https://github.com/hyperf/hyperf/pull/7648) 为 `nacos` 支持阿里云 `mse` 签名。

## 优化

- [#7676](https://github.com/hyperf/hyperf/pull/7676) 优化 `hyperf/tracer` 组件的客户端标签写法。

## 修复

- [#7703](https://github.com/hyperf/hyperf/pull/7703) 修复 `hyperf/cache` 内存适配器不支持默认 `ttl` 的问题。
- [#7705](https://github.com/hyperf/hyperf/pull/7705) 修复 `Hyperf\Database\Query\Builder::eachById` 无法正常使用的问题。
- [#7712](https://github.com/hyperf/hyperf/pull/7712) 修复 `classes` 等扫描缓存解析失败导致报错的问题。

# v3.1.66 - 2026-01-25

## 新增

- [#7667](https://github.com/hyperf/hyperf/pull/7667) [#7672](https://github.com/hyperf/hyperf/pull/7672) 为 `hyperf/grpc-client` 新增参数 `client_count` 用来支持创建多个客户端。
- [#7668](https://github.com/hyperf/hyperf/pull/7668) 为 `Hyperf\Guzzle\PoolHandler` 新增参数 `$isCookiePersistent` 用来支持是否持久化 `Cookies`。
- [#7670](https://github.com/hyperf/hyperf/pull/7670) 为文件生成命令增加 `--path` 选项，用来指定生成位置。
- [#7684](https://github.com/hyperf/hyperf/pull/7684) 新增方法 `Hyperf\Database\Schema\Builder::withoutForeignKeyConstraints()`。
- [#7693](https://github.com/hyperf/hyperf/pull/7693) 为 `ForeignKeyDefinition` 新增方法 `cascadeOnDelete()`, `restrictOnDelete()`, `noActionOnDelete()`, `restrictOnUpdate()`, `nullOnUpdate()`, `noActionOnUpdate()`。
- [#7698](https://github.com/hyperf/hyperf/pull/7698) 新增方法 `Pool::flushAll()` 和 `PoolFactory::flushAll()`。
- [#7699](https://github.com/hyperf/hyperf/pull/7699) 为 `QueryBuilder` 新增方法 `whereJsonContainsKey()`, `orWhereJsonContainsKey()`, `whereJsonDoesntContainKey()`, `orWhereJsonDoesntContainKey()`。

## 优化

- [#7675](https://github.com/hyperf/hyperf/pull/7675) 为 `metric` 组件删除多余的定时器清理代码。
- [#7687](https://github.com/hyperf/hyperf/pull/7687) 为 `JSON-RPC` 组件增加默认捕获 `ValidationException` 的逻辑。

## 修复

- [#7669](https://github.com/hyperf/hyperf/pull/7669) 修复 `PostgreSQL` 组件中 `boolean` 格式问题。
- [#7682](https://github.com/hyperf/hyperf/pull/7682) 修复 `Blueprint::foreign()` 返回值注释错误。
...
- [#7649](https://github.com/hyperf/hyperf/pull/7649) 为 `Hyperf\Metric\Listener\OnBeforeHandle` 移除 `AfterExecute` 的事件监听。
...
- [#7601](https://github.com/hyperf/hyperf/pull/7601) 增加异步队列消息执行结果枚举 `Hyperf\AsyncQueue\Result`。

## 优化

- [#7604](https://github.com/hyperf/hyperf/pull/7604) 优化异步队列 `AsyncQueueMessage` 注解支持定义在 `protected` 和 `private` 方法上。
- [#7611](https://github.com/hyperf/hyperf/pull/7611) 为 `Console::command()` 增加注释 `@param-closure-this`，用来获得更好的 `ide` 代码提示。
- [#7615](https://github.com/hyperf/hyperf/pull/7615) 把异步队列的消费者进程中的 `$queue` 参数重命名为 `$pool`。

## 修复

- [#7596](https://github.com/hyperf/hyperf/pull/7596) 修复使用验证器时，使用了不存在的场景值，导致报错的问题。

# v3.1.63 - 2025-10-30

## 修复

- [#7536](https://github.com/hyperf/hyperf/pull/7536) 修复 `Hyperf\Validation\ValidationRuleParser::parseParameters()` 不支持 `PHP 8.4` 的问题。
- [#7543](https://github.com/hyperf/hyperf/pull/7543) 修复 `nacos` 组件代码没有严格符合 `psr4` 标准的问题。
- [#7554](https://github.com/hyperf/hyperf/pull/7554) 修复参数 `Redis` 集群 `context` 参数在没有设置密码时，不生效的问题。
- [#7561](https://github.com/hyperf/hyperf/pull/7561) 重置 `Nacos\Application` 的构造函数参数名。

## 新增

- [#7553](https://github.com/hyperf/hyperf/pull/7553) 对 `hyperf/testing` 组件增加 `PATCH` 方法的支持。

## 优化

- [#7511](https://github.com/hyperf/hyperf/pull/7511) 优化 `hyperf/database` 的注释。
- [#7537](https://github.com/hyperf/hyperf/pull/7537) 对 `swagger` 组件，使用 `priorityMiddleware` 替代 `middleware`。
...
- [#7501](https://github.com/hyperf/hyperf/pull/7501) 为 `Hyperf\Macroable\Macroable::macro()` 增加引用验证，避免使用引用导致代码执行结果不达预期的情况。
...
- [#6556](https://github.com/hyperf/hyperf/pull/6556) 在数据库组件时，允许为模型参数复制表达式。**_但这不是一个标准用法，尽量不要使用_**
