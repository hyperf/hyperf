...
> DAG 會按照儘可能早的原則排程任務。嘗試將 B 點的耗時調整為 2 秒，會發現 B 和 G 一起完成。

## 訪問前步結果

每一個任務可以接收一個數組引數，陣列中包含所有前置依賴的結果。`DAG` 執行完畢後，也會返回一個同樣結構的陣列，包含每一步的執行結果。
...
`\Hyperf\Dag\Dag` 本身也實現了 `\Hyperf\Dag\Runner` 介面，所以可以巢狀使用。
...
## 控制併發數
`\Hyperf\Dag\Dag` 類提供了 `setConcurrency(int n)` 方法控制最大併發數。預設為 10。
