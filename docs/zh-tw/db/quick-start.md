...
可以在 PhpStorm 中安裝 [Hyperf Query](https://plugins.jetbrains.com/plugin/33333-hyperf-query) 外掛，為 Hyperf 查詢構造器提供資料庫整合支援。它配合 DataGrip 為資料庫 schema、資料表、視圖和欄位提供自動完成，主要功能如下：

- 查詢構造器和 Schema 構造器方法中 schema、資料表、視圖、欄位的自動完成
- 遷移檔案（migration）中的自動完成
...
安裝方式：`Preferences` > `Plugins` > `Marketplace` 搜尋 **Hyperf Query**，點擊 **Install Plugin** 安裝。安裝後建議先完成資料來源配置，並在外掛設定中過濾要使用的資料來源。
