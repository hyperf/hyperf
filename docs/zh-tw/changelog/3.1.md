...
- [#6509](https://github.com/hyperf/hyperf/pull/6509) 修復 `Crontab` 元件的 `Schedule::call()` 方法不支援 `array` 引數。

## 最佳化

- [#6511](https://github.com/hyperf/hyperf/pull/6511) 最佳化非同步佇列訊息體結構 `Hyperf\AsyncQueue\JobMessage`。
- [#6513](https://github.com/hyperf/hyperf/pull/6513) 使用 `tracer` 元件時，使用 `Hyperf\Coroutine\Coroutine` 代替 `Hyperf\Engine\Coroutine`。
