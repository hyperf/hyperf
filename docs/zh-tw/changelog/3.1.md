...
- [#7791](https://github.com/hyperf/hyperf/pull/7791) 修復 `Hyperf\Validation\Concerns\ValidatesAttributes::validateMultipleOf()` 不支援最新 `brick/math` 的問題。

## 最佳化

- [#7791](https://github.com/hyperf/hyperf/pull/7791) 最佳化 `Hyperf\Tracer\Adapter\Reporter\HttpClientFactory` 的 `Content-Type` 資料型別為 `string`。

# v3.1.70 - 2026-06-15

## 最佳化

- [#7761](https://github.com/hyperf/hyperf/pull/7761) 最佳化方法 `Hyperf\Coordinator\Timer::tick()` 的執行邏輯運行於 `Hyperf\Coroutine\wait()` 方法中，以確保執行在獨立的協程環境裡。
- [#7761](https://github.com/hyperf/hyperf/pull/7761) 最佳化方法 `wait()` 優先檢查 `Hyperf\Coroutine\Waiter` 是否存在容器中。
...
- [#7734](https://github.com/hyperf/hyperf/pull/7734) 修復了在長生命週期環境下使用 Redis::pipeline() 時記憶體洩漏的 bug。
- [#7736](https://github.com/hyperf/hyperf/pull/7736) 修復了 RedisDriver 中 keys 方法在 sMembers 返回非陣列值時無法返回陣列的 bug。
- [#7737](https://github.com/hyperf/hyperf/pull/7737) 修復了 PostgresProcessor 在 columns 為 null 時無法處理索引和外來鍵的 bug。
- [#7745](https://github.com/hyperf/hyperf/pull/7745) 修復了使用 Swoole 6.2.0 時 getUri()->getHost() 返回伺服器 IP 而非域名的 bug，原因是 getUriFromGlobals() 中 server_addr 的優先順序高於 header['host']。
...
- [#7739](https://github.com/hyperf/hyperf/pull/7739) 修復使用 `nacos` 時，因為服務不存在，導致無法啟動的問題。
- [#7740](https://github.com/hyperf/hyperf/pull/7740) 修復 `Aws\Handler\GuzzleV6\GuzzleHandler` 找不到的問題。

## 最佳化

- [#7741](https://github.com/hyperf/hyperf/pull/7741) 最佳化方法 `Hyperf\Amqp\Message\Message::getTypeString()` 避免成員變數與 `getter` 衝突的問題。統一使用 `getter` 讀取。

# v3.1.67 - 2026-02-24

## 新增

- [#7459](https://github.com/hyperf/hyperf/pull/7459) 使用驗證器規則 `exists` 和 `unique` 時，支援使用形如 `App\\Model\\User` 的模型寫法。
- [#7555](https://github.com/hyperf/hyperf/pull/7555) 新增 `Hyperf\Crontab\Mutex\RedisServerMutexByNodeName`。
- [#7607](https://github.com/hyperf/hyperf/pull/7607) 為 `hyperf/database` 增加新的模型自定義型別轉換器 `AsCollection`。
- [#7648](https://github.com/hyperf/hyperf/pull/7648) 為 `nacos` 支援阿里雲 `mse` 簽名。

## 最佳化

- [#7676](https://github.com/hyperf/hyperf/pull/7676) 最佳化 `hyperf/tracer` 元件的客戶端標籤寫法。

## 修復

- [#7703](https://github.com/hyperf/hyperf/pull/7703) 修復 `hyperf/cache` 記憶體介面卡不支援預設 `ttl` 的問題。
- [#7705](https://github.com/hyperf/hyperf/pull/7705) 修復 `Hyperf\Database\Query\Builder::eachById` 無法正常使用的問題。
- [#7712](https://github.com/hyperf/hyperf/pull/7712) 修復 `classes` 等掃描快取解析失敗導致報錯的問題。

# v3.1.66 - 2026-01-25

## 新增

- [#7667](https://github.com/hyperf/hyperf/pull/7667) [#7672](https://github.com/hyperf/hyperf/pull/7672) 為 `hyperf/grpc-client` 新增引數 `client_count` 用來支援建立多個客戶端。
- [#7668](https://github.com/hyperf/hyperf/pull/7668) 為 `Hyperf\Guzzle\PoolHandler` 新增引數 `$isCookiePersistent` 用來支援是否持久化 `Cookies`。
- [#7670](https://github.com/hyperf/hyperf/pull/7670) 為檔案生成命令增加 `--path` 選項，用來指定生成位置。
...
- [#7675](https://github.com/hyperf/hyperf/pull/7675) 為 `metric` 元件刪除多餘的定時器清理程式碼。
- [#7687](https://github.com/hyperf/hyperf/pull/7687) 為 `JSON-RPC` 元件增加預設捕獲 `ValidationException` 的邏輯。

## 修復

- [#7669](https://github.com/hyperf/hyperf/pull/7669) 修復 `PostgreSQL` 元件中 `boolean` 格式問題。
...
- [#7632](https://github.com/hyperf/hyperf/pull/7632) 增加 `thecodingmachine/graphqlite:^8.1` 的支援。
...
- [#7601](https://github.com/hyperf/hyperf/pull/7601) 增加非同步佇列訊息執行結果列舉 `Hyperf\AsyncQueue\Result`。

## 最佳化

- [#7604](https://github.com/hyperf/hyperf/pull/7604) 最佳化非同步佇列 `AsyncQueueMessage` 註解支援定義在 `protected` 和 `private` 方法上。
- [#7611](https://github.com/hyperf/hyperf/pull/7611) 為 `Console::command()` 增加註釋 `@param-closure-this`，用來獲得更好的 `ide` 程式碼提示。
- [#7615](https://github.com/hyperf/hyperf/pull/7615) 把非同步佇列的消費者程序中的 `$queue` 引數重新命名為 `$pool`。
...
- [#7536](https://github.com/hyperf/hyperf/pull/7536) 修復 `Hyperf\Validation\ValidationRuleParser::parseParameters()` 不支援 `PHP 8.4` 的問題。
- [#7543](https://github.com/hyperf/hyperf/pull/7543) 修復 `nacos` 元件程式碼沒有嚴格符合 `psr4` 標準的問題。
- [#7554](https://github.com/hyperf/hyperf/pull/7554) 修復引數 `Redis` 叢集 `context` 引數在沒有設定密碼時，不生效的問題。
- [#7561](https://github.com/hyperf/hyperf/pull/7561) 重置 `Nacos\Application` 的建構函式引數名。

## 新增

- [#7553](https://github.com/hyperf/hyperf/pull/7553) 對 `hyperf/testing` 元件增加 `PATCH` 方法的支援。

## 最佳化

- [#7511](https://github.com/hyperf/hyperf/pull/7511) 最佳化 `hyperf/database` 的註釋。
- [#7537](https://github.com/hyperf/hyperf/pull/7537) 對 `swagger` 元件，使用 `priorityMiddleware` 替代 `middleware`。

# v3.1.62 - 2025-10-11

## 新增

- [#7469](https://github.com/hyperf/hyperf/pull/7469) 使用引數驗證器時，支援透過場景值覆寫引數驗證規則。
- [#7531](https://github.com/hyperf/hyperf/pull/7531) 為指令碼 `migrate:rollback` 新增引數 `batch`。
- [#7545](https://github.com/hyperf/hyperf/pull/7545) 執行指令碼最後自動執行 `CoordinatorManager::until(Constants::WORKER_EXIT)->resume();`。

## 修復

- [#7547](https://github.com/hyperf/hyperf/pull/7547) 修復 `kafka` 消費者配置成員變數 `$pool` 時，無法正常使用的問題。

# v3.1.61 - 2025-09-04

## 新增

- [#7467](https://github.com/hyperf/hyperf/pull/7467) 為 Nacos SDK 支援 v2 和 v3 版本。
- [#7476](https://github.com/hyperf/hyperf/pull/7476) 為 `Hyperf\Support\Fluent` 新增 trait `InteractsWithData` 支援。
...
- [#7487](https://github.com/hyperf/hyperf/pull/7487) 新增模型對 `ArrayObject` 型別的轉換器 `Hyperf\Database\Model\Casts\AsArrayObject`。
- [#7492](https://github.com/hyperf/hyperf/pull/7492) 為 `Hyperf\Pipeline\Pipeline` 新增 trait `Macroable` 的支援。
...
- [#7501](https://github.com/hyperf/hyperf/pull/7501) 為 `Hyperf\Macroable\Macroable::macro()` 增加引用驗證，避免使用引用導致程式碼執行結果不達預期的情況。
...
- [#6556](https://github.com/hyperf/hyperf/pull/6556) 在資料庫元件時，允許為模型引數複製表示式。**_但這不是一個標準用法，儘量不要使用_**
