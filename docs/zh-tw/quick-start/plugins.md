...
- 配置鍵：`config()` 輔助函式與 `ConfigInterface::get()/has()` 的鍵索引、補全與跳轉（支援 3.1+ 子目錄與點號檔名）
...
- 檢視模板：`view()`、`RenderInterface::render()` 等模板名的補全與跳轉（點語法 + `pkg::name` 名稱空間）
- AOP 切面：`#[Aspect]` 註解與 `AbstractAspect` 屬性中 `'FQN::method'` 字串的跳轉與方法名補全
- 快取監聽器：`#[Cacheable(listener: "...")]` 與 `DeleteListenerEvent` 監聽器名的補全與互跳
- DI 介面繫結：懸停介面時文件彈窗顯示當前生效的實現類連結
- Crontab：`callback` 方法名補全與跳轉；`rule` 表示式懸停顯示最近 5 次執行時間
- Hyperf 頂級選單：程式碼生成（`gen:*`）與常用命令（`migrate`、`start`、`describe:routes` 等）一鍵在內建 Terminal 執行
- 命令類行標記：`Hyperf\Command\Command` 子類類名旁的執行按鈕，點選直接執行命令
