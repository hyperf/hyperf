...
- `config/autoload/aspects.php` 檔案需要存在(空檔案也行), 否則會報錯
- 如果使用不同環境的 `.env` 檔案, Dockerfile 中執行 `php bin/hyperf.php` 快取前, 需要執行 `cp .env.${APP_ENV}`, 否則可能因為 `.env` 中配置連線無法連線上報錯
- 自定義 process 需要修改 `public function isEnable($server): bool` 方法, 相容 2.0 版本的 api
- 自定義註解，如果使用了自定義的收集器 `Collector`，則需要將對應的 `Collector` 配置到 `annotations.scan.collectors` 中，因為開發模式下，會根據檔案時間戳判斷檔案是否修改，然後重新收集註解。
