...
The `init-proxy.sh` script has been removed due to the adjustment of the underlying logic of AOP in version 2.0, so you need to remove the `"init-proxy' in `scripts.post-autoload-dump` from `composer.json`. sh"` to execute the statement, and modify the command in `post-autoload-dump` to the `rm -rf runtime/container` statement.
...
Since we want to upgrade to the 2.0 version of the component, and the original skeleton project depends on the 1.1.x version of the component by default, we need to make some adjustments to the constraints of the dependency, and change the dependencies of all the original Hyperf components to `~1.1.0 ` is changed to `~2.0.0`, and you also need to change the `phpstan/phpstan` version dependency to `^0.12`. After the modification, you need to run `composer update` to upgrade the dependencies to version 2.0.
...
return [
    // use the prod value for production
...
Version 2.0 adjusted the processing logic of HTTP routing exception behaviors such as route not found (404), request method mismatch (405), etc., and changed it to throw a sub-exception class of `Hyperf\HttpMessage\Exception\HttpException`, and then ExceptionHandler is used to uniformly manage these exceptions and handle corresponding responses, so that users can also get a consistent response return experience by throwing corresponding exceptions, but since ExceptionHandler is a user-managed mechanism, the default in version 1.1 Skeleton configures an `App\Exception\Handler\AppExceptionHandler` class to handle exceptions and return the 500 status code to the client, so you need to use the `Hyperf\HttpServer provided by version 2.0 to handle HttpException \Exception\Handler\HttpExceptionHandler` is configured in the `config/autoload/exceptions.php` configuration file, and make sure to be in front of the `App\Exception\Handler\AppExceptionHandler` configuration, the following configuration example omits extraneous configuration, as shown below :
...
After you complete the configuration of ExceptionHandler, you can directly access a route that does not exist. If the status code of the response is `404`, it can be understood that the modification of this configuration is completed.
...
Paths under this configuration are considered static file routes, so if `/` is configured, it will cause all interfaces to be considered file paths, resulting in interface 404.
...
If you encounter any problems during or after the upgrade, please go to [Github Issue](https://github.com/hyperf
