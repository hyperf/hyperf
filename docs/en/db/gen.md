...
| :----------------: | :----: | :-------------------------------- ---------: | :------------------------------------------------ ---------: |
| --pool | string | `default` | Connection pool, the script will create it according to the current connection pool configuration |
...
| --table-mapping | array | `[]` | Add mapping relationship for table name -> model such as ['users:Account'] |
| --ignore-tables | array | `[]` | Table names that don't need to generate models e.g. ['users'] |
| --with-comments | bool | `false` | Whether to add field comments |
| --property-case | int | `0` | field type 0 snake case 1 camel case |
